...
layout(location = 2) in vec4 f_position_light_space;
...
//vec3 get_light_dir(DirectionalLight light, vec3 position) {
...
float get_shadow(vec4 f_position_light_space, vec3 light_dir) {
     vec3 proj_position = f_position_light_space.xyz / f_position_light_space.w;
...
//     f_color = vec4(vec3(point_depth), 1.0);
     ...
     vec3 light_dir = normalize(-uniforms.light_dir);
     ...
     float diffuse_shadow = get_shadow(f_position_light_space, light_dir);
